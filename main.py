...
# Три вспышки по секунде после кадра. Само моргание длится шесть секунд,
# поэтому идёт по таймеру, а не в потоке съёмки: иначе на эти шесть секунд
# встал бы и ответ веб-странице, и обработка нажатия кнопки на плате.
# Съёмка от моргания не задерживается вообще.
LED_BLINKS = 3
LED_PERIOD_MS = 1000
...
def run_camera_server(ip, ap_ip=""):
    ...
    def _led_step(t):
        if led_left["n"] <= 0:
            ...
            return
        led_left["n"] -= 1
        led.value(LED_ON if led_left["n"] % 2 else LED_OFF)

    def led_blink(times=LED_BLINKS):
        """Подтверждение, что кадр снят. Нужно в первую очередь для кнопки
        на плате: там браузера нет и сказать о съёмке больше нечем.

        Гасит и зажигает таймер, а не цикл со сном: за шесть секунд
        моргания сервер должен успеть отдать превью, а кнопка — сработать
        ещё раз, если на неё нажмут."""
        led.value(LED_ON)
        led_left["n"] = times * 2 - 1
        led_timer.init(period=LED_PERIOD_MS, mode=Timer.PERIODIC,
                       callback=_led_step)

    def camera_off():
        """Гасит камеру немедленно, не дожидаясь простоя.

        Вызывается перед выводом на панель. Кадр к этому моменту уже снят
        и лежит в памяти, камера не нужна — а вот держать её включённой
        во время обновления экрана нельзя: OV5640 при fb_count=1 гоняет
        кадры непрерывно и ест больше всех на плате, обновление панели
        длится 5.6с в 4 градациях, и раньше эти два потребителя работали
        одновременно. От сетевого адаптера это проходило незаметно, от
        повербанка плата вставала после второго снимка.

        Плата за это — холодный старт сенсора (~1.2с) на следующем
        снимке. Он прячется внутрь задержки автоспуска, если она задана."""
        cam = cam_state["cam"]
        if cam is not None:
            cam.deinit()
            cam_state["cam"] = None
    ...
    def capture_now():
        ...
        state["seq"] += 1
        # Камеру будим ДО отсчёта, а не после: холодный старт сенсора —
        # это ~1.2с (HW-замерено), и они прячутся внутрь задержки, вместо
        # того чтобы к ней прибавляться.
        cam = ensure_cam()
        if state["delay_ms"] > 0:
            time.sleep_ms(state["delay_ms"])
            cam_state["last_use"] = time.ticks_ms()
    ...
    run_server(routes, on_idle=on_idle, idle_interval_sec=0.2)
